...
public:
    ...
    : m_position(position) , m_radius(radius), m_center(center){
        ...
        memoryUsageCol += sizeof(SphereCollider);
    }
    ...
    void SetupMesh(float radius, int resolution) {
        ...
        memoryUsageCol += vertices.size() * sizeof(SphereColVertex);
        memoryUsageCol += sizeof(GLuint); // VAO
        memoryUsageCol += sizeof(GLuint); // VBO
    }

    void Draw(Shader &wireframe) override {
        if(sphereCollider) {
            wireframe.Use();
            //wireframe.setMat4("m_Model", m_modelMatrix);


            // std::cerr << "Position: (" << m_position.x << ", " << m_position.y << ", " << m_position.z << ")" << std::endl;
            // std::cerr << "Radius: " << m_radius << std::endl;
