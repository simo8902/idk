...
public:
    ...
    virtual std::shared_ptr<GameObject> clone() const override {
        ...
        // Deep copy of child objects
        for (const auto& childObj : m_objects) {
            std::shared_ptr<GameObject> clonedChild = childObj->clone(); // Recursively clone children
            clonedObject->addObject(clonedChild);
        }
